...
public class ARSites {
	...
	public void create(final String id, final String name, final double lon,
		...
		AsyncHttpUtils asyncHttpUtils = new AsyncHttpUtils(mApiKey, mTime,
				mSignature);
			...
			public void onResponse(HttpResponse serverResponse) {
				...
				PayloadExtractor<ARSite> extractor = new PayloadExtractor<ARSite>() {
					...
					public ARSite extract(HttpResponse httpResponseFromExtractor) {
						...
						BasicResponse addSiteResponse = responseHandler
								.handleResponse(httpResponseFromExtractor,
										BasicResponse.class);

						if (addSiteResponse.getSuccess() == true) {
							return new ARSiteImpl(id, mApiKey, mTime,
									mSignature);

						} else {
							throw new ARException(
									"Successfully communicated with the server, but couldn't create the site. The site Id "
											+ id
				...
				ARResponse<ARSite> addArSiteResponse = ARResponse.from(
		...
		asyncHttpUtils.doGet(HttpUtils.PARWORKS_API_BASE_URL
				+ HttpUtils.ADD_SITE_PATH, parameterMap, callback);
		...
		HttpCallback callback = new HttpCallback() {
			...
			public void onResponse(HttpResponse serverResponse) {
				...
				PayloadExtractor<ARSite> extractor = new PayloadExtractor<ARSite>() {
					...
					public ARSite extract(HttpResponse httpResponseFromExtractor) {
						ARResponseHandler responseHandler = new ARResponseHandlerImpl();
						BasicResponse nearbySites = responseHandler
								.handleResponse(httpResponseFromExtractor,
										BasicResponse.class);
						...
						if (nearbySites.getSuccess() == true) {
							...
							throw new ARException(
									"Successfully communicated with the server, but couldn't create the site. The site "
											+ id
											+ " is probably already in use.");
						}
					}
	...
	private void near(String lat, String lon, String max, String radius,
		...
		HttpCallback callback = new HttpCallback() {
			...
			public void onResponse(HttpResponse serverResponse) {
				...
				PayloadExtractor<List<ARSite>> extractor = new PayloadExtractor<List<ARSite>>() {
					...
					@Override
					public List<ARSite> extract(
							HttpResponse httpResponseFromExtractor) {
						ARResponseHandler responseHandler = new ARResponseHandlerImpl();
						NearbySitesResponse nearbySites = responseHandler
								.handleResponse(httpResponseFromExtractor,
										NearbySitesResponse.class);

						if (nearbySites.getSuccess() == true) {
							List<SiteInfo> sitesInfo = nearbySites.getSites();
							List<ARSite> nearbySitesList = new ArrayList<ARSite>();
							for (SiteInfo info : sitesInfo) {
								nearbySitesList.add(new ARSiteImpl(
										info.getId(), mApiKey, mTime,
										mSignature));
							}
							return nearbySitesList;
						} else {
							throw new ARException(
									"Successfully communicated with the server, but the server was unsuccessful in finding nearby sites.");
						}
					}

				};
				ARResponse<List<ARSite>> nearbySitesARResponse = ARResponse
		...
		asyncHttpUtils.doGet(HttpUtils.PARWORKS_API_BASE_URL
				+ HttpUtils.NEARBY_SITE_PATH, parameterMap, callback);
	}
	...
	public void getExisting(String id, final ARListener<ARSite> listener) {
				...
				PayloadExtractor<ARSite> extractor = new PayloadExtractor<ARSite>() {
					...
					public ARSite extract(HttpResponse httpResponseFromExtractor) {
						...
						if (siteInfoResponse.getSuccess() == true) {
							return new ARSiteImpl(siteInfoResponse.getSite()
									.getId(), mApiKey, mTime, mSignature);
						} else {
							throw new ARException(
									"Successfully communicated with the server, but the server couldn't get a site. Maybe it doesn't exist.");
						}
					}

				};
				ARResponse<ARSite> nearbySitesARResponse = ARResponse.from(
						serverResponse, extractor);
		...
		asyncHttpUtils.doGet(HttpUtils.PARWORKS_API_BASE_URL
				+ HttpUtils.GET_SITE_INFO_PATH, parameterMap, callback);
